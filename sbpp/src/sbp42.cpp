...
void Sbp42::DT(const double* f, double* dtf,
  ...
  auto pinv {1.0/GetQuadratureWeights(length)/h};
  ...
  int idx = (length-4)*stride;
  dtf[idx] = -(q3*f[(length-3)*stride]*pinv[length-3] -
               q5*f[(length-1)*stride]*pinv[length-1] -
               q2*f[(length-5)*stride]*pinv[length-5] +
               q1*f[(length-6)*stride]*pinv[length-6]);

  idx = (length-3)*stride;
  dtf[idx] = -(-q3*f[(length-4)*stride]*pinv[length-4] +
  ...
  idx = (length-1)*stride; 
  dtf[idx] = -(q5*f[(length-4)*stride]*pinv[length-4] +
               q4*f[(length-3)*stride]*pinv[length-3] -
               q3*f[(length-2)*stride]*pinv[length-2] +
               0.5*f[(length-1)*stride]*pinv[length-1]);

  for(int i = 4; i < length-4; ++i) {
    dtf[i*stride] = -(q1*f[(i-2)*stride]*pinv[i-2] -
                      q2*f[(i-1)*stride]*pinv[i-1] +
                      q2*f[(i+1)*stride]*pinv[i+1] -
                      q1*f[(i+2)*stride]*pinv[i+2]);
   }
